               ...
               @foreach($time_slot as $ts)
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @foreach($time_slot as $ts)
                        @forelse (\App\Models\Trip::where('ts_id', $ts->id)->where('bus_id', $bus->id)->where('day_id', $day->id)->get() as $lecture)
                                <td style="border:1px solid #343a40 ; text-align: center" class="text-center font-weight-bold">{{$lecture->name }}<br><span style="font-size: 12px ">({{$lecture->driver->name }})</span></td>
                            @empty
