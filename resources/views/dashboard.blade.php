...
<nav class="sb-topnav navbar navbar-expand navbar-dark bg-dark" style="background-color: black">
    ...
    <h1 style="font-size: 25px ; font-weight: bold" class="navbar-brand ps-3" href="#">
        {{\App\Models\Setting::where("key","school_name")->pluck("value")[0]}}
       </h1>
...
<div id="layoutSidenav">
    ...
    @php
        $type=  \App\Models\Type_User::where( "id",auth()->user()->type_id)->pluck("type")[0];
           ...
           $end_first_term=  \App\Models\Setting::where("key","end_first_term")->pluck("value")[0];
        $end_first_term = \Carbon\Carbon::createFromFormat("Y-m-d",$end_first_term);
        $Current_year=\Carbon\Carbon::createFromFormat("Y-m-d",\Carbon\Carbon::now()->format("Y-m-d"));
        $semster=0;
        if ($Current_year->gt($end_first_term)){
            $semster="the second semester";
        }
        if ($Current_year->lte($end_first_term)){
            $semster="the first semester";
        }

        $year=\App\Models\Setting::where("key", "current_session")->pluck("value")[0];
    @endphp
                            ...
                            <div class="card  card-statistics h-100">
                                <div class="btn-primary card-body">
                                    <div class="clearfix">
                                        <div class="float-left ">
<span class="text-white">
<i class="fas fa-bus highlight-icon" aria-hidden="true"></i>
</span>
                                        </div>
                                        <div class="float-right text-right">
                                            <h5 class="card-text text-white">Trip Number :</h5>
                                            <h4 class="text-white">{{\App\Models\Trip::
     where('user_id', '=',$id)
               ->where('year', '=',$year)
                ->where('semester', '=',$semster)->count()}}</h4>
                                        </div>
                                    </div>
                                    <p class="text-muted pt-3 mb-0 mt-2 border-top">
                                    <h6 class="text-white"><a href="{{route('Invoices_index')}}">View Details</a></h6>
                                ...
                                <table class="table datatable-button-html5-columns">
                                    ...
                                    <tr>
                                        @php
                                            ...
                                            $time_slot=\Illuminate\Support\Facades\DB::table('trips as l')
              ->select('ts.full' ,'ts.id')
              ->distinct()
              ->join('time_slots as ts', 'ts.id', '=', 'l.ts_id')
              ->where('l.user_id', '=',$id)
               ->where('l.year', '=',$year)
                ->where('l.semester', '=',$semster)
         ...
         $days = \App\Models\Day::all();
                                    ...
                                    @foreach($days as $day)
                                        <tr>
                                            ...
                                            @foreach($time_slot as $ts)
                                                @forelse (\App\Models\Trip::where('ts_id', $ts->id)->where('user_id',$id)->where('semester',$semster)->where('year',$year)->where('day_id', $day->id)->get() as $lecture)
                                                    <td style="border:1px solid #343a40 ; text-align: center" class="text-center font-weight-bold">{{$lecture->name }}<br><span style="font-size: 12px ">({{$lecture->type }})</span></td>
                                                @empty
